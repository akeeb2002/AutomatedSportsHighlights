...
import argparse
import csv
from video_transition import create_transition  # Import the transition function
...
def apply_smoothing(cap, frame_filter, frame_number, fps):
    # Get the next frame with value 1
    next_frame_number = next((frame for frame in frame_filter if frame > frame_number and frame_filter[frame] == 1), None)

    if next_frame_number:
        gap = next_frame_number - frame_number
        # If the gap is larger than 5 frames, apply transition
        if gap > 5:
            # Create a fade transition between the current frame and the next frame with value 1
            transition_frames = create_transition(cap, frame_number, next_frame_number, transition_type='fade', duration_frames=gap)
...
def main():
    ...
    while True:
        ...
        if not frame_filter or frame_filter.get(frame_number, 0) == 1:
            ...
            # Check for smoothing if the current frame is a 1
            if frame_filter.get(frame_number, 0) == 1:
                smoothing_frames = apply_smoothing(input_video, frame_filter, frame_number, fps)
                if smoothing_frames:
                    # Add the transition frames to the output video
                    for smoothed_frame in smoothing_frames:
                        output_video.write(smoothed_frame)
